...
# 必須列
BASE_NEEDED = ["実行日", "台番号"]
SRC_COLS = ["大当り回数", "最終スタート", "出玉数"]
TARGET_COL = "出玉数noボナ回転率"
...
df_latest["出玉数"] = pd.to_numeric(df_latest["出玉数"], errors="coerce")

# === 指定の計算式（大当り回数が0の台だけ）===
# 出玉数noボナ回転率 = 最終スタート ÷ ( - 出玉数 ) × 250
denom = -df_latest["出玉数"]              # -出玉数
num = df_latest["最終スタート"]
mask = (df_latest["大当り回数"] == 0) & (~num.isna()) & (~denom.isna()) & (denom > 0)

rate = np.where(mask, (num / denom) * 250, np.nan)

# 必要なら丸め（例：小数2桁）
df_latest[TARGET_COL] = pd.to_numeric(rate, errors="coerce").round(2)
