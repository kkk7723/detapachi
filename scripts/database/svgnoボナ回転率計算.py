...
# 必須列
BASE_NEEDED = ["実行日", "台番号"]
SRC_COLS = ["大当り回数", "最終スタート", "svg差枚"]
TARGET_COL = "svg差枚noボナ回転率"
...
# 数値化
df_latest["大当り回数"] = pd.to_numeric(df_latest["大当り回数"], errors="coerce")
df_latest["最終スタート"] = pd.to_numeric(df_latest["最終スタート"], errors="coerce")
df_latest["svg差枚"] = pd.to_numeric(df_latest["svg差枚"], errors="coerce")

# === 指定の計算式（大当り回数が0 または None/空白の台だけ）===
# 出玉数noボナ回転率 = 最終スタート ÷ ( - 出玉数 ) × 250
denom = -df_latest["svg差枚"]              # -出玉数
num = df_latest["最終スタート"]

mask = ((df_latest["大当り回数"] == 0) | (df_latest["大当り回数"].isna())) \
       & (~num.isna()) & (~denom.isna()) & (denom > 0)

rate = np.where(mask, (num / denom) * 250, np.nan)

# ✅ 小数点1桁に丸める
df_latest[TARGET_COL] = pd.Series(rate).round(1)
